jQuery(document).ready(function() {
  ...
  jQuery('.beautyplus-admin-dashboard #beautyplus-wp-notices:not(.__A__WP_Notices_Container)').show();
  ...
  jQuery(document).on( 'mouseover', '.__A__MainMenuV > li, .__A__MainMenuV li ul li', function() {
    ...
    $submenu = $menuItem.find( '> .beautyplus-header-submenu' );
  ...
  function navigationResize() {
    if (self === top) {
      ...
      jQuery('#overflow').addClass('beautyplus-header-submenu');
      ...
      if (jQuery('#overflow').height() > (jQuery(window).height()-100)) {
        jQuery('#overflow').addClass('beautyplus-more-double');
      }
  ...
  var BeautyPlusAjaxCounter;

  window.BeautyPlusAjax = function(type, message) {
    type = typeof type !== 'undefined' ? type : "clear";
    message = typeof message !== 'undefined' ? message : BeautyPlusGlobal.i18n.wait;
    ...
    if ('clear' === type ) {
      ...
      clearTimeout(BeautyPlusAjaxCounter);
    ...
    if ('success' === type) {
      ...
      clearTimeout(BeautyPlusAjaxCounter);
      BeautyPlusAjaxCounter = setTimeout(function() {
        BeautyPlusAjax('hide', message);
      }, 4000);
    }
  ...
  */
  ...
  jQuery('body').on('click', '.__A__Desktop_Control', function(e) {
    e.preventDefault();
    jQuery('title').text(jQuery('title').text() + 'Beauty-asterisk-' + jQuery(this).data('do'));
  });
  ...
  jQuery(".__A__Left_Search, .dashicons-search").on( "click", function() {
    ...
    jQuery(".beautyplus-search-1--overlay").addClass('beautyplus-search-1--overlay-show');

    setTimeout(function() {   jQuery(".beautyplus-search-input").focus(); }, 500);


  });

  jQuery("#beautyplus-search-1--close-button").on( "click", function() {
    jQuery("body").css({overflow:'auto'});
    jQuery(".beautyplus-search-1--overlay").removeClass('beautyplus-search-1--overlay-show');
    jQuery(".beautyplus-search-input").attr('placeholder',  jQuery(".beautyplus-search-input").val()).val('');
  });


  jQuery(".beautyplus-search-input").on("keyup", function(e) {
  ...
  jQuery(".beautyplus-search-input").doWithDelay("keyup", function(e) {

    if (!jQuery(".beautyplus-search-1--overlay").hasClass('beautyplus-search-1--overlay-show')) {
      jQuery("body").css({overflow:'hidden'});
    ...
    xhr = jQuery.post( BeautyPlusGlobal.ajax_url, {
      _wpnonce: jQuery('input[name=_wpnonce]').val(),
      _wp_http_referer: jQuery('input[name=_wp_http_referer]').val(),
      _asnonce: BeautyPlusGlobal._asnonce_search,
  ...
  if (BeautyPlusGlobal.refresh>9000) {
    setInterval(function(last) {
      notifications();
    }, BeautyPlusGlobal.refresh);
  }
  ...
  function notifications(lasttime) {
    ...
    jQuery.ajax({
      type: 'POST',
      dataType: 'json',
      url: BeautyPlusGlobal.ajax_url,
      data: {
        _wpnonce: jQuery('input[name=_wpnonce]').val(),
        _wp_http_referer: jQuery('input[name=_wp_http_referer]').val(),
        _asnonce: BeautyPlusGlobal._asnonce_notifications,
        action: 'beautyplus_ajax',
  ...
  function slideRevelWidth() {
    window.device_width = BeautyPlusGlobal.reactors_tweaks_window_size; // 770
  ...
  jQuery('#inbrowser, #beautyplus-frame').on('load', function() {
    jQuery("#inbrowser").show();
    jQuery("#inbrowser--loading").removeClass('d-flex').addClass('d-none');
    jQuery(".__A__Trig_Close").removeClass('d-none');
  });
  ...
  if ("1" === BeautyPlusGlobal.reactors_tweaks_adminbar_hotkey) {
