jQuery(document).ready(function() {
  ...
  detectHash(BeautyPlusGlobal._admin_url + "comment.php?action=editcomment&c=HASH");
    ...
    jQuery.post( BeautyPlusGlobal.ajax_url, {
      ...
      if (1 === r.status) {
        if ('untrash' === r.state || 'unspam' === r.state) {
          ...
          jQuery(".__A__CommentStatus > span", t).removeClass('badge-danger').addClass('badge-success').text(BeautyPlusComments.approved);
          jQuery(".__A__CommentStatusButton", t).text(BeautyPlusComments.unapprove).removeClass('__A__CommentStatusButton_Unapprove').addClass('__A__CommentStatusButton_Approve').attr('data-state', 'unapprove');
        } else if ('unapprove' === r.state) {
          jQuery(".__A__CommentStatus > span", t).removeClass('badge-secondary').addClass('badge-danger').text(BeautyPlusComments.unapproved);
          jQuery(".__A__CommentStatusButton", t).text(BeautyPlusComments.approve).removeClass('__A__CommentStatusButton_Unapprove').addClass('__A__CommentStatusButton_Approve').attr('data-state', 'approve');
        } else {
      ...
      if (1 === r.status) {
        jQuery.each(r.id, function(i, item) {
          ...
          if ('untrash' === item.state || 'unspam' === item.state) {
            ...
            jQuery(".__A__CommentStatus > span", t).removeClass('badge-danger').addClass('badge-success').text(BeautyPlusComments.approved);
            jQuery(".__A__CommentStatusButton", t).text(BeautyPlusComments.unapprove).attr('data-state', 'unapprove');
          } else if ('unapprove' === item.state) {
            jQuery(".__A__CommentStatus > span", t).removeClass('badge-success').addClass('badge-danger').text(BeautyPlusComments.unapproved);
            jQuery(".__A__CommentStatusButton", t).text(BeautyPlusComments.approve).attr('data-state', 'approve');
