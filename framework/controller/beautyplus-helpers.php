...
class BeautyPlus_Helpers extends BeautyPlus {
  ...
  */
  ...
  public static function ajax_nonce( $get_nonce = false, $action = 'beautyplus-general') {
    if (TRUE === $get_nonce) {
  ...
  */
  ...
  public static function submenu($panel) {
    ...
    $panel_parent = 'beautyplus-'.$panel;

    foreach ($menu AS $_m_k => $_m)  {

      if (isset($_m['parent']) && (1 === $_m['active']) && ($panel_parent === $_m['parent'])) {
        $secure_url = BeautyPlus_Helpers::secure_url($panel, md5($_m_k), array('go' => $_m_k, 'status'=>'none', 'action'=>'none'));

        if ($_m_k === BeautyPlus_Helpers::get('go')) {
  ...
  */
  ...
  /**
  * Generate admin link for BeautyPlus panels
  *
  * @since  1.0.0
  */

  public static function admin_page($segment = '', $others = array()) {
    return admin_url( "admin.php?page=beautyplus&segment=" . $segment ."&". implode('&', array_map(
  ...
  */
  ...
  public static function frame ($page) {
    echo BeautyPlus_View::run('core/frame', array( 'page' => esc_url($page) ));
    return;
  }
  ...
  */
  ...
  public static function grouped_time ($date)  {
    ...
    if (intval(BeautyPlus_Helpers::strtotime('now', 'z')) === intval(BeautyPlus_Helpers::strtotime($date, 'z'))) {
      return array('key'=>'d'.BeautyPlus_Helpers::strtotime($date, 'z'), 'title'=>esc_html__('Today', 'beautyplus'));
    }

    if (intval((BeautyPlus_Helpers::strtotime('now', 'z')-1)) === intval(BeautyPlus_Helpers::strtotime($date, 'z'))) {
      return array('key'=>'d'.BeautyPlus_Helpers::strtotime($date, 'z'), 'title'=> esc_html__('Yesterday', 'beautyplus'));
    }

    if (BeautyPlus_Helpers::strtotime($date,'Ymd') >=  BeautyPlus_Helpers::strtotime('monday this week','Ymd')) {
      return array('key'=>'w'.BeautyPlus_Helpers::strtotime($date,'mW'), 'title'=> esc_html__('This Week', 'beautyplus'));
    }

    if ((BeautyPlus_Helpers::strtotime($date, 'Ymd') <=  BeautyPlus_Helpers::strtotime('monday this week', 'Ymd')) AND (BeautyPlus_Helpers::strtotime($date, 'Ymd') >=  BeautyPlus_Helpers::strtotime('monday last week','Ymd'))) {
      return array('key'=>'w'.BeautyPlus_Helpers::strtotime($date,'mW'), 'title'=> esc_html__('Last Week', 'beautyplus'));
    }

    if ((BeautyPlus_Helpers::strtotime($date, 'Ymd') <=  BeautyPlus_Helpers::strtotime('monday last week', 'Ymd')) AND (BeautyPlus_Helpers::strtotime($date, 'Ymd') >=  BeautyPlus_Helpers::strtotime('first day of this month','Ymd'))) {
      return array('key'=>'m'.BeautyPlus_Helpers::strtotime($date,'m'), 'title'=> esc_html__('This Month', 'beautyplus'));
    }

    if ((BeautyPlus_Helpers::strtotime($date, 'Ymd') <  BeautyPlus_Helpers::strtotime('first day of this month', 'Ymd')) AND (BeautyPlus_Helpers::strtotime($date, 'Ymd') >=  BeautyPlus_Helpers::strtotime('first day of January','Ymd'))) {
      return array('key'=>'m'.BeautyPlus_Helpers::strtotime($date,'m'), 'title'=> date_i18n('F', strtotime($date)));
    }

    return array('key'=>'y'.BeautyPlus_Helpers::strtotime($date,'Y'), 'title'=> esc_html__('Year ', 'beautyplus') . BeautyPlus_Helpers::strtotime($date,'Y'));
  ...
  */
  ...
  public static function api_pagination($_this, $query) {
    ...
    BeautyPlus_Admin::$api = array('query'=>$query);
  ...
  */
  ...
  public static function is_desktop_app() {
    ...
    $user_agent = BeautyPlus_Helpers::clean($_SERVER ['HTTP_USER_AGENT']);

    $platform = 'none';

    if (stripos( 'BeautyPlus (Mac)', $user_agent)!==false) {
      $platform = 'Mac';
    } else	if (stripos($user_agent, 'BeautyPlus (Windows)')!==false) {
      $platform = 'Windows';
    }
  ...
  */
  ...
  public static function option_color($args = array()) {
    ...
    <div class="float-left"><input name="' . esc_attr($args['name']). '" type="text" value="' . esc_attr($args['value']). '" class="__A__Reactors_Color_' . esc_attr($args['name']). ' ' . esc_attr($args['class']). ' beautyplus-color-field" data-default-color="' . esc_attr($args['value']). '"
