...
class BeautyPlus_Events extends BeautyPlus {
  ...
  */
  ...
  public static function notifications(){
    ...
    $lastid                 = 0;
    $now                    = time();
    $last                   = intval(BeautyPlus_Helpers::post('lasttime', 0));
    $notifications_lasttime = intval ( BeautyPlus::option('notifications_lasttime_' . absint(get_current_user_id()) , 0) );
    $last_timestamp         = BeautyPlus_Helpers::timestamp(intval(BeautyPlus_Helpers::post('lasttime', 0)));


    $query = $wpdb->prepare(
      "SELECT count(*) count FROM {$wpdb->prefix}beautyplus_events WHERE type IN (1,2,3,4,5,11,12) AND event_id > %d",
    ...
    if ( 0 === $notifications_lasttime OR -1 === $last) {
      ...
      $query = $wpdb->prepare("SELECT event_id FROM {$wpdb->prefix}beautyplus_events WHERE type IN (1,2,3,4,5,11,12)  ORDER BY event_id DESC LIMIT %d", 1);
      $_last = $wpdb->get_var($query);

      BeautyPlus::option('notifications_lasttime_' . absint(get_current_user_id()) , absint($_last), 'set');
    }
    ...
    if (-2 === $last)
    {
      ...
      $where = ' event_id > ' . esc_sql(intval(sanitize_text_field(BeautyPlus_Helpers::post('lastid', 0))));
    }


    $limit = max(intval($count)+10, 10);
    $query          = $wpdb->prepare("SELECT event_id, user, type, id, extra, time FROM {$wpdb->prefix}beautyplus_events WHERE type IN (1,2,3,4,5,11,12) AND $where ORDER BY event_id DESC LIMIT 0, %d", $limit);
    ...
    foreach ($_notifications AS $notify) {
      ...
      switch ($notify->type)
      {
        ...
        $notifications[$notify->event_id] = array(
          'time'    => $notify->time,
          'type'    => $notify->type,
          'title'   => sprintf (esc_html__('New order', 'beautyplus')),
          'details' => array(
            'order_id'             => $notify->id,
        ...
        if (intval(BeautyPlus_Helpers::post('lasttime', time())) >0 && strtotime($notify->time) > intval(BeautyPlus_Helpers::post('lasttime', time()))) {
          $notifications[$notify->event_id]['status'] = 'new';
          $alerts[] = array(
            'title' => esc_html__('New order', 'beautyplus'),
            ...
            'link'  => BeautyPlus_Helpers::admin_page('orders', array( ))
          );

          $sounds[0] = BeautyPlus_Public.'sounds/notification.mp3';
        ...
        $notifications[$notify->event_id] = array(
          'time'    => $notify->time,
          'type'    => $notify->type,
          'title'   => sprintf (esc_html__("%s has commented to %s", 'beautyplus'), $comment->comment_author, $post->post_title),
          'details' => array(
        ...
        if (intval(BeautyPlus_Helpers::post('lasttime', time())) >0 && strtotime($notify->time) > intval(BeautyPlus_Helpers::post('lasttime', time()))) {
          $notifications[$notify->event_id]['status'] = 'new';
          $alerts[] = array(
            'title' => esc_html__('New comment', 'beautyplus'),
            'body'  => sprintf (esc_html__("%s has commented to %s", 'beautyplus'), $comment->comment_author, $post->post_title),
            'link'  => BeautyPlus_Helpers::admin_page('comments', array()) . "#" . $comment->comment_ID
          );
        ...
        $notifications[$notify->event_id] = array(
          'time'    => $notify->time,
          'type'    => $notify->type,
          'title'   => sprintf (esc_html__("%s &mdash; Coupon usage limit has been reached", 'beautyplus'), strtoupper($extra[0])),
          'details' => array(
            'coupon_code' => $extra[0],
            'coupon_id'   => $notify->id,
    ...
    echo json_encode( array( 'background'=>true, 'status'=>1, 'result'=> BeautyPlus_View::run('core/notifications',  array( 'notifications' => $notifications ) ), 'count' => $count, 'title' => $title, 'lastid' => $lastid, 'time'=> $now, 'alerts' => $alerts, 'today_sales'=>$today_sales, 'sounds'=>$sounds));
  ...
  public static function notification_count() {
    ...
    $query = $wpdb->prepare("SELECT count(*) count FROM {$wpdb->prefix}beautyplus_events WHERE type IN (1,2,3,4,5,11,12) AND event_id > %d", $notifications_lasttime) ;
    $count = $wpdb->get_var($query);
  ...
  */
  ...
  public static function search() {
    ...
    $term = BeautyPlus_Helpers::post('q', '');

    if ($term) {
      wp_die( BeautyPlus_View::run('core/search',  array( 'term' =>  $term)) );
    } else {
      wp_die();
    }
  ...
  */
  ...
  public static function get_title($notification_count) {
    ...
    $option             = BeautyPlus::option('feature-badge', 0);
  ...
  */
  ...
  public static function add($data = array()) {
    ...
    $insert = $wpdb->insert( $wpdb->prefix."beautyplus_events",
    array(
      'user'   => $data['user'],
...
public static function save_post_shop_order( $order_id ) {
  ...
  $_GET['start_date'] = BeautyPlus_Helpers::strtotime('now', "Y-m-d");
  $_GET['end_date']   = BeautyPlus_Helpers::strtotime('now', "Y-m-d");
  ...
  set_transient( 'today_sales', floatval(floor($report_data->total_sales)) , BeautyPlus_Helpers::strtotime('tomorrow 00:00:00', 'U')-BeautyPlus_Helpers::strtotime('now', 'U') );
