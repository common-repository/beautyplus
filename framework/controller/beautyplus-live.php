...
class BeautyPlus_Live extends BeautyPlus {
  ...
  */
  ...
  public static function pulse() {
    ...
    switch (BeautyPlus_Helpers::post('t')) {

      // Product
      case "p":

      $ID      = intval( BeautyPlus_Helpers::post('i', 0));
      ...
      $data['type']  = 10;
      $data['id']    = 1;
      $data['extra'] = sanitize_text_field(BeautyPlus_Helpers::post('i', ''));
  ...
  */
  ...
  public static function add_request($data = array()) {
    ...
    if (!isset($data['date'])) {
      $data['date'] = BeautyPlus_Helpers::time();
    }
    ...
    $insert = $wpdb->insert( $wpdb->prefix."beautyplus_requests",
    array(
      'session_id' => self::get_session(),
      'visitor'    => $data['visitor'],
      'year'       => BeautyPlus_Helpers::strtotime($data['date'],'y'),
      'month'      => BeautyPlus_Helpers::strtotime($data['date'],'m'),
      'week'       => BeautyPlus_Helpers::strtotime($data['date'],'W'),
      'day'        => BeautyPlus_Helpers::strtotime($data['date'],'d'),
      'date'       => $data['date'],
      'ip'         => $data['ip'],
      'type'       => $data['type'],
      'id'         => $data['id'],
...
public static function get_visitor() {
  ...
  if ($visitor === 0) {
    if (!isset($_COOKIE["beautyplus-u"])) {

      $visitor = md5(uniqid().rand(1,100000).WC_Geolocation::get_ip_address().time());
      setcookie("beautyplus-u", $visitor, time()+(365*24*60*60), COOKIEPATH, COOKIE_DOMAIN);

    } else {
      $visitor = sanitize_key($_COOKIE["beautyplus-u"]);
    }
...
public static function get_session() {
  ...
  if (!isset($_COOKIE["beautyplus-session"])) {
    $session_id = md5(uniqid().rand(1,100000).WC_Geolocation::get_ip_address().time());
    setcookie("beautyplus-session", $session_id, time()+(20*60), COOKIEPATH, COOKIE_DOMAIN);
  } else {

    $_session_id = BeautyPlus_Helpers::clean($_COOKIE["beautyplus-session"]);
