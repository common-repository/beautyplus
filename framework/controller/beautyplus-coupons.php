...
class BeautyPlus_Coupons extends BeautyPlus {
	...
	*/
	...
	public static function run() {
		...
		wp_enqueue_script("beautyplus-coupons",  BeautyPlus_Public . "js/beautyplus-coupons.js");
	...
	*/
	...
	public static function filter($filter = false) {
		...
		if ('' !== BeautyPlus_Helpers::get('status', '')) {
			$filter['post_status'] = BeautyPlus_Helpers::get('status', null);
		}

		if ('' !== BeautyPlus_Helpers::get('s', '')) {
			$filter['s'] = BeautyPlus_Helpers::get('s', '');
		}
	...
	*/
	...
	public static function index($filter = false) {
		...
		switch ( $mode = ( !empty($filter['mode']) ? absint($filter['mode']) : BeautyPlus::option('mode-beautyplus-coupons', 1) ) ) {

			// Woocommerce Native
			case 99:
			if (!BeautyPlus_Admin::is_full()) {
				BeautyPlus_Helpers::frame( admin_url( 'edit.php?post_type=shop_coupon' ) );
			} else {
			...
			if ('trash' === BeautyPlus_Helpers::get('status', 'publish')) {
				...
				if ('' !== $filter['s'])
				{
					...
					$coupons =  (array)self::get_coupons( BeautyPlus_Helpers::get('status', 'publish'), $filter,  $filter['page'] );
				}
			}

			if (95 === $mode) {
				echo BeautyPlus_View::run('coupons/list-95', array('counts' => wp_count_posts('shop_coupon'), 'count' => $coupons['count'], 'iframe_url' => BeautyPlus_Helpers::get_submenu_url(BeautyPlus_Helpers::get('go')) ));
			} else {
				echo BeautyPlus_View::run('coupons/list-'. $mode,  array( 'count' => $coupons['count'], 'per_page' => $filter['limit'], 'coupons' => $coupons['result'], 'counts' => wp_count_posts('shop_coupon'), 'ajax' =>   BeautyPlus_Helpers::is_ajax() ));
			}
			break;
		}
	}
	...
	*/
	...
	public static function stats($coupon_id = 0, $args = array()) {
		...
		if (0 === $coupon_id) {
			$coupon_id	=	absint( BeautyPlus_Helpers::get('id', 0) );
		}
		...
		foreach ($orders AS $_orders) {
			...
			$time       = BeautyPlus_Helpers::grouped_time( $order->get_date_created() );
		...
		if (isset($args['return'])) {
			...
			echo BeautyPlus_View::run('coupons/stats',  array( 'orders' => $data, 'coupon'=> $coupon, 'counts' => wp_count_posts('shop_coupon') ));
		}
	...
	*/
	...
	public static function ajax() {
		...
		$coupon_id = BeautyPlus_Helpers::post('id', 0) ;
		...
		switch ($do)
		{
			...
			$filter['s']           = BeautyPlus_Helpers::post('q', '');
			...
			foreach ($ids AS $id)
			{
				...
				if ( 0 === $coupon->ID ) {
					return BeautyPlus_Ajax::error("Invalid Coupon");
				}
				...
				if ( 'private' === BeautyPlus_Helpers::post( 'state' ) OR 'publish' === BeautyPlus_Helpers::post( 'state' ) ) {
					$state = ('private' === BeautyPlus_Helpers::post('state')) ? 'private' : 'publish';
			...
			return BeautyPlus_Ajax::success('Coupon has been changed', array('id'=> $success, 'new'=>$state));
			...
			BeautyPlus_Helpers::ajax_nonce( TRUE, 'beautyplus-coupons--onoff-' . $coupon_id  );
			...
			$state = ('false' === BeautyPlus_Helpers::post('state')) ? 'private' : 'publish';
			...
			return BeautyPlus_Ajax::success('Coupon has been changed', array('id'=>$coupon->ID, 'new'=>$state));
	...
	*/
	...
	public static function delete()
	{
		...
		$coupon_id	=	intval( BeautyPlus_Helpers::get('id', 0) );
		$force = ( 'true' ===  BeautyPlus_Helpers::get('forever', 'false') ) ? 'true' : 'false';

		BeautyPlus_Helpers::ajax_nonce(TRUE, $coupon_id);

		if ($coupon_id === 0) {
			wp_die('Error');
		}

		if ( 'true' ===  BeautyPlus_Helpers::get('untrash', 'false') ) {
			$result = wp_untrash_post( $coupon_id );

			wp_safe_redirect(  BeautyPlus_Helpers::admin_page('coupons') );
