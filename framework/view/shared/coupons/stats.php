...
<div id="beautyplus-coupons-stats">

  <div class="__A__Stats">
    <div class="beautyplus-gp">
      <div class="row d-flex align-items-center">
        <div class="col">
          <div class="__A__Code"><span class="badge badge-pill badge-black"><?php echo esc_html($coupon['code']); ?></span></div>
          <?php  printf( esc_html__('Created at %s', 'beautyplus'), $coupon['created_at']); ?></div>
          <div class="col"><div class="__A__Big"><?php echo esc_html( $coupon['usage_count']);?></div> <?php echo esc_html__('TIMES USED', 'beautyplus'); ?></div>
          <div class="col"><div class="__A__Big"><?php echo wc_price($coupon['total_discount']);?></div> <?php echo esc_html__('TOTAL DISCOUNT', 'beautyplus'); ?></div>
          <div class="col"><div class="__A__Big"><?php echo wc_price($coupon['total_sales']);?></div> <?php echo esc_html__('TOTAL REVENUE', 'beautyplus'); ?></div>
          <div class="col"><div class="__A__Big"><?php if (0 < $coupon['total_sales']) {
            echo sprintf('%.2f', (100*$coupon['total_discount']/$coupon['total_sales']));
          } else {
            echo 0;
          } ?>%</div> <?php echo esc_html__('DISCOUNT PERCENT', 'beautyplus'); ?></div>
    ...
    <div class="__A__Stats">
      <div class="beautyplus-gp text-center">
    ...
    <div id="beautyplus-coupons-1" class="beautyplus-gp">
      <?php foreach ($orders AS $order_group) { ?>
        <h4><?php echo esc_attr( $order_group['title']); ?></h4>
        <?php foreach ($order_group['orders'] AS $order) { ?>
          <div class="beautyplus-list--item row d-flex align-items-center">
            <div class="col col-lg-1 beautyplus-orders--item-badge text-center"><?php
            switch ($order['status']) {
          ...
          <div class="col col-lg-3 __A__Coupons_Items">
            <?php foreach ($order['line_items'] AS $item) { ?>
              <?php echo BeautyPlus_Helpers::product_image(intval($item['product_id']), intval($item['quantity']), 'width: 50px'); ?>
            <?php } ?>
          </div>
          <div class="col col-lg-2 text-center">
            <span class="beautyplus-orders--item-price"><?php echo wc_price($order['discount']); ?></span>
            <br><span class="beautyplus-orders--item-currency"><?php echo esc_html__('DISCOUNT', 'beautyplus'); ?></span>
          </div>
          <div class="col col-lg-2 text-center">
            <span class="beautyplus-orders--item-price"><?php echo wc_price($order['order_total']); ?></span>
            <br><span class="beautyplus-orders--item-currency"><?php echo esc_html__('TOTAL', 'beautyplus'); ?></span>
          </div>
          <div class="col-lg-1 text-right">
            <a href="<?php echo admin_url( 'post.php?post=' . $order['order_id']. '&action=edit&beautyplus_hide' );?>" class="__A__Button1 trig"><?php echo esc_html__('View Order', 'beautyplus'); ?></a>
          </div>
        </div>
      <?php } ?>
    <?php } ?>
