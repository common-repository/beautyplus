...
<?php if (!isset($ajax)) { ?>
  <div class="__A__Widget_Options">
    <h1 class="float-left">
      <?php esc_html_e('Last Activities', 'beautyplus'); ?>
    </h1>
    <div class="__A__Widget_Options_AutoHide float-left">
      <ul>
        <li><a class="__A__Widget_Settings_<?php echo esc_attr($args['id']);?>_Range<?php if ('online' === $args['range']) echo esc_attr( ' __A__Selected' ); ?>" data-range='online' href="javascript:;"><?php esc_html_e('Online', 'beautyplus'); ?></a></li>
          <li><a class="__A__Widget_Settings_<?php echo esc_attr($args['id']);?>_Range<?php if ('all' === $args['range']) echo esc_attr( ' __A__Selected' ); ?>" data-range='all' href="javascript:;"><?php esc_html_e('All', 'beautyplus'); ?></a></li>
          </ul>
        </div>
        <div class="__A__Clear_Both"></div>
      </div>
      <div class="__A__Widget_Lastactivity_container __A__Range_<?php echo esc_attr($args['range']);?>">
        <div class="__A__EmptyTable <?php if (0 < count($result)) { echo esc_attr( ' d-none' ); } else { echo esc_attr( 'd-flex' ); }?> align-items-center justify-content-center text-center">
          <div><span class="dashicons dashicons-marker"></span><br><?php esc_html_e('No online visitor', 'beautyplus'); ?></div>
        </div>
      <?php } ?>
      <?php foreach ($result AS $session) { ?>
        <div class="__A__Widget_Lastactivity_row animated fadeInUp d-flex align-items-center __A__Time_<?php echo date("Hi", strtotime($session['date'])); ?> __A__Widget_Lastactivity_Sess_<?php echo esc_attr($session['id']); ?>" id="__A__Widget_Lastactivity_Sess_<?php echo esc_attr($session['id']); ?>">
          <div class="__A__I1">
            <?php if (300 > (BeautyPlus_Helpers::strtotime('now', 'U')-BeautyPlus_Helpers::strtotime($session['date'], 'U'))) { ?>
              ...
              <?php foreach ($session['views'] AS $views_key => $r) { ?>
                <div class="__A__I_O_Container">
                  ...
                  <?php } elseif (4 === $r['type']) {  ?>
                    ...
                    <div class="__A__Widget_Lastactivity_T4"  data-toggle="tooltip" data-placement="bottom" title="<?php echo esc_html(sprintf( esc_html__( '%s (%s) has been added to cart' , 'beautyplus') , $r['details']['name'], strip_tags ( wc_price($r['details']['price'] ) ) )); ?>">+</div>
                  <?php } elseif (5 === $r['type']) {  ?>
                    <img src="<?php echo get_the_post_thumbnail_url($r['details']['id']); ?>" class="__A__Product_Image" >
                    <div class="__A__Widget_Lastactivity_T5"   data-toggle="tooltip" data-placement="bottom" title="<?php echo esc_html( sprintf( esc_html__( '%s (%s) has been removed from cart' , 'beautyplus') , $r['details']['name'], strip_tags ( wc_price($r['details']['price'] ) ) ) ); ?>">-</div>
                  <?php } elseif (6 === $r['type']) {  ?>
                    <div class="__A__Widget_Lastactivity_T6"  data-toggle="tooltip" data-placement="bottom" title="<?php esc_html_e('Checkout', 'beautyplus'); ?>"><?php echo esc_html( get_woocommerce_currency_symbol() ); ?></div>
                  <?php } elseif (7 === $r['type']) {  ?>
                    <div class="__A__Widget_Lastactivity_T7"   data-toggle="tooltip" data-placement="bottom" title="<?php esc_html_e('Homepage', 'beautyplus'); ?>" ><span class="dashicons dashicons-admin-site"></span></div>
                  <?php } elseif (10 === $r['type']) {  ?>
                    <div class="__A__Widget_Lastactivity_T10"  data-toggle="tooltip" data-placement="bottom" title="<?php esc_html_e('Search:', 'beautyplus'); ?> <?php echo esc_html(implode(", ", $r['details']['term'])); ?>" ><span class="dashicons dashicons-search"></span></div>
