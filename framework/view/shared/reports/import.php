...
<div class="__A__Reports_Import">
  ...
  <div class="__A__Reports_Import_Intro">
    <h2><?php esc_html_e('Import Tool', 'beautyplus'); ?></h2>
    <p><?php esc_html_e('This tool allows you to export your past reports to BeautyPlus.', 'beautyplus'); ?>
      <br>
      <?php esc_html_e('Please start it when your store is least busy since it will run many queries in the background.', 'beautyplus'); ?>
    </p>
    <p class="text-danger"><?php esc_html_e('Please do not close this window during import.', 'beautyplus'); ?></a>
      <br><br>
    </div>
    ...
    <div class="text-center">
      <button class="button-danger __A__Reports_Start_Import"><?php esc_html_e('Start import', 'beautyplus'); ?></button>
    </div>
  </div>
  ...
  jQuery(document).ready(function($) {
    ...
    jQuery('.__A__Reports_Start_Import').on('click', function() {
      beautyplusImport('<?php echo esc_attr(BeautyPlus_Helpers::strtotime('today', 'Y-m-01'))?>', 'import-date', '<?php echo esc_attr(BeautyPlus_Helpers::strtotime('today', 'Y-m-01'))?>');
    ...
    function beautyplusImport(id, type, last) {
      jQuery.post( BeautyPlusGlobal.ajax_url, {
        ...
        if (r.status === 1) {
          ...
          beautyplusImport(r.message.date, r.message.type, id);
