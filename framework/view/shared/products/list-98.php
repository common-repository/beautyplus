...
<?php if (0 < count( $products )) {   ?>
  <h2 class="badge badge-black badge-pill __A__Badge_Big_Title"><?php esc_html_e('Products', 'beautyplus'); ?></h2>
  <div class="__A__List_M1" id="beautyplus-products-1">
          ...
          <div class="liste  row d-flex align-items-center">
            ...
            <div class="__A__Col_3 col-2 align-middle text-center"  data-colname="Stock">
              <div class="__A__Stocks1" id="__A__Stock_<?php echo esc_attr($product['id']);   ?>">
                <?php if (true === $product['managing_stock']) {   ?>
                  <?php if (0 <  intval($product['stock_quantity'])) {
                    ...
                    echo wp_kses_post( '<span class="badge badge-danger">' . esc_html__('Out Of Stock', 'beautyplus'). '</span>' );
                  }?>
                <?php } else {  ?>
              ...
              <div class="__A__Stocks text-left __A__Display_None">
                ...
                <br>
                <input type="checkbox" name="unlimited" data-id="<?php echo  esc_attr($product['id']); ?>" class="__A__StockAjax" <?php if ((true !== $product['managing_stock'] && true === $product['in_stock']) OR (true === $product['managing_stock'] && 9999 === $product['stock_quantity'])) echo esc_attr( ' checked' ); ?>> <?php esc_html_e('Unlimited', 'beautyplus'); ?><br>
                <input type="checkbox" name="outofstock" data-id="<?php echo  esc_attr($product['id']); ?>"  class="__A__StockAjax" <?php if ('1' <> $product['in_stock']) echo esc_attr( ' checked' ); ?>> <?php esc_html_e('Out of Stocks', 'beautyplus'); ?><br>
              </div>
            </div>
            ...
            <div class="containerx">
              <div class="row">
                <div class="col-12 col-sm-12 text-right __A__Product_Actions">
                  <a href="<?php echo admin_url( 'post.php?post=' . esc_attr($product['id']). '&action=edit&beautyplus_hide' );?>" class="__A__StopPropagation trig"><?php esc_html_e('Edit product', 'beautyplus'); ?></a>
                </div>
              </div>
            </div>
          </div>
