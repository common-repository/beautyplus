...
class Widgets__Hourly extends BeautyPlus_Widgets {
  ...
  public static function run ( $args = array(), $settings = array() ) {
    ...
    switch ($range) {
      ...
      BeautyPlus_Helpers::strtotime('first day of January','Ym'), BeautyPlus_Helpers::strtotime('today', 'Ym')
    ), ARRAY_A
  );
	...
	$result[] = array(
		...
		BeautyPlus_Helpers::strtotime('first day of this month'), BeautyPlus_Helpers::strtotime('today','m')
	))
);
    ...
    $labels = array();
    for ($i = 1; $i< 13; ++$i) {
      $labels[] = "'".BeautyPlus_Helpers::strtotime("2019-$i-01", 'M')."'";
    }
    ...
    $result = $wpdb->get_results(
      ...
      BeautyPlus_Helpers::strtotime('first day of this month','Ymd'), BeautyPlus_Helpers::strtotime('today', 'Ymd')
    ), ARRAY_A
  );
		...
		$result[] = array(
			...
			FROM {$wpdb->prefix}beautyplus_requests
			WHERE date >= %s AND month=%d",
			BeautyPlus_Helpers::strtotime('today'), BeautyPlus_Helpers::strtotime('today','m')
		))
	);
...
if (BeautyPlus_Helpers::is_ajax() OR isset( $args['ajax'] ))  {
  return BeautyPlus_View::run('widgets/hourly',  array( 'args' => $args, 'max' => $max, 'labels'=>$labels, 'ajax' => true, 'results' => $_result ));
} else {
  echo BeautyPlus_View::run('widgets/hourly',  array( 'args' => $args, 'max' => $max,'labels'=>$labels, 'results' => $_result ));
}
