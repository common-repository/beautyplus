...
class Widgets__Lastactivity extends BeautyPlus_Widgets {
	...
	public static function run ( $args = array(), $settings = array() ) {
		...
		$time = BeautyPlus_Helpers::strtotime('now - '.absint(BeautyPlus::option('feature-refresh', 30)).' seconds');
		...
		switch ($range) {
			...
			if ($args['counter'] === -2 OR $args['counter'] === 0) {
				$time = BeautyPlus_Helpers::strtotime('now - 24 hours');
			}
			...
			$__last_sessions = $wpdb->get_results(
				...
				$time, BeautyPlus_Helpers::strtotime('now')
			), ARRAY_A );
			break;
			...
			if ($args['counter'] === -2) {
				$time = BeautyPlus_Helpers::timestamp(strtotime('now - 5 minutes'));
			...
			$__last_sessions = $wpdb->get_results(
				...
				BeautyPlus_Helpers::strtotime($time, 'm'),	BeautyPlus_Helpers::strtotime("now", 'm'), $time, BeautyPlus_Helpers::strtotime('now')
			), ARRAY_A );
		...
		if (BeautyPlus_Helpers::is_ajax() OR isset( $args['ajax'] ))  {
			return array("updated"=> $last_sessions, "off_time" =>  BeautyPlus_Helpers::strtotime('-5 minutes', "Hi"), "list"=>BeautyPlus_View::run('widgets/lastactivity',  array('args' => $args, 'ajax'=>1, 'result' => $sessions)));
		} else {
			echo BeautyPlus_View::run('widgets/lastactivity',  array('args' => $args,  'result' => $sessions));
		}
	...
	private static function get_session($session_ids) {
		...
		$query = "SELECT {$wpdb->prefix}beautyplus_requests.* FROM {$wpdb->prefix}beautyplus_requests WHERE type IN (1,2,4,5,6,7,10,17) AND month = %d AND (session_id='-1' ";
			...
			$__result = $wpdb->get_results(
				$wpdb->prepare($query,
				BeautyPlus_Helpers::strtotime(current_time('mysql'), 'm')
				)
			);
		...
		private static function get_user($user, $ip = 0) {
			if (is_numeric($user)) {
				...
				if (!is_wp_error($__visitor)) {
					...
					$visitor = esc_html__('Visitor', 'beautyplus');
					$city = $ip;
				}
			} else {
				$visitor = esc_html__('Visitor', 'beautyplus');
