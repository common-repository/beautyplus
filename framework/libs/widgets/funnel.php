...
class Widgets__Funnel extends BeautyPlus_Widgets {
  ...
  public static function run ( $args = array(), $settings = array() ) {
    ...
    $data['results']      = BeautyPlus_Reports::beautyplus_data(array('range'=>'daily'));
    ...
    if (BeautyPlus_Helpers::is_ajax() OR isset( $args['ajax'] ))  {
      return BeautyPlus_View::run('widgets/funnel',  array('args' => $args, 'data' => $data));
    } else {
      echo BeautyPlus_View::run('widgets/funnel',  array('args' => $args, 'data' => $data));
    }
  }
